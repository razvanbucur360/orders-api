...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	...
	if err != nil {
		return fmt.Errorf("Error while JSON encoding the order: %w", err)
	}
...
var ErrNotExists = errors.New("order deos not exist")
...
func (r *RedisRepo) FindByID(ctx context.Context, id uint64) (model.Order, error) {
	...
	if errors.Is(err, redis.Nil){
		...
		return model.Order{}, fmt.Errorf("Error occured: %w", err)
	}
	...
	if err != nil {
		return model.Order{}, fmt.Errorf("Error unmarshalling JSON: %w", err)
	}
...
func (r *RedisRepo) Update(ctx context.Context, order model.Order) error {
	...
	if err != nil {
		return fmt.Errorf("Error JSON encoding the data: %w", err)
	}
